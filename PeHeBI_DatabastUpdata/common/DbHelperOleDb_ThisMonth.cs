...
namespace PeHeBI_DatabastUpdata.common
{
    ...
    /// </summary>
    public abstract class DbHelperOleDb_ThisMonth
    {
        //数据库连接字符串(web.config来配置)，可以动态更改connectionString支持多数据库.		
        public static string connectionString = global_variable.ConnectionString_ThisMonthpat;



        public DbHelperOleDb_ThisMonth()
        {
        }
        ...
        public static int GetMaxID()
        {
            using (var conn = new OleDbConnection(connectionString))
            {
                using (var cmd = conn.CreateCommand())
                {
                    ...
                    {
                        if (ex .Message== "对象不能从 DBNull 转换为其他类型。")
                        {
                            return 0;
