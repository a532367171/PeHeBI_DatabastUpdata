...
namespace PeHeBI_DatabastUpdata.common
{
    ...
    /// </summary>
    public abstract class DbHelperOleDb_LastMonth
    {
        //数据库连接字符串(web.config来配置)，可以动态更改connectionString支持多数据库.		
        public static string connectionString = global_variable.ConnectionString_LastMonthpat;
        public DbHelperOleDb_LastMonth()
        {
        }
        ...
        public static int GetMaxID()
        {
            using (var conn = new OleDbConnection(connectionString))
            {
                using (var cmd = conn.CreateCommand())
                {
                    ...
                    cmd.CommandType = CommandType.Text;
                    //string xxxx = TableConvention.Resolve(typeof(Access_sRecords_Model));
                    cmd.CommandText = "select Max(ID) from sRecords";
                    conn.Open();


                    object x = Convert.ToInt32(cmd.ExecuteScalar());
