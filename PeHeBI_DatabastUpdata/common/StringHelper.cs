...
namespace PeHeBI_DatabastUpdata.common
{
    public class StringHelper
    {
        ...
        public static void logW(string strLogFilePath, string Mas)
        {
            try
            {
                ...
                var logContent = string.Format("Mes: {0}----{1} \r\n", DateTime.Now.ToString(), Mas);


                //var logContentBytes = Encoding.Default.GetBytes(logContent);

                //var logContentBytes = Encoding.Unicode.GetBytes(logContent);

                var logContentBytes = Encoding.UTF8.GetBytes(logContent);



                //由于设置了文件共享模式为允许随后写入，所以即使多个线程同时写入文件，也会等待之前的线程写入结束之后再执行，而不会出现错误
                using (FileStream logFile = new FileStream(strLogFilePath, FileMode.OpenOrCreate, FileAccess.Write, FileShare.Write))
                {
